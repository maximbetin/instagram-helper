...
def test_main_import_error() -> None:
    ...
    with (
        patch("run.InstagramHelperGUI", side_effect=ImportError("Module not found")),
...
def test_main_gui_error() -> None:
    ...
    with (
        patch("run.InstagramHelperGUI", return_value=mock_gui),
        patch("builtins.print") as mock_print,
        patch("sys.exit") as mock_exit,
    ):
        main()
...
def test_main_gui_creation_error() -> None:
    ...
    with (
        patch("run.InstagramHelperGUI", side_effect=Exception("Failed to create GUI")),
        patch("builtins.print") as mock_print,
        patch("sys.exit") as mock_exit,
    ):
        main()
...
def test_main_import_error_different_message() -> None:
    ...
    with (
        ...
        # Check error messages
        mock_print.assert_any_call(
            "Error importing required modules: No module named 'tkinter'"
        )
        mock_print.assert_any_call(
            "Please ensure all dependencies are installed: pip install -e ."
        )
...
def test_main_gui_error_different_message() -> None:
    ...
    with (
        patch("run.InstagramHelperGUI", side_effect=Exception("Tkinter not available")),
        patch("builtins.print") as mock_print,
        patch("sys.exit") as mock_exit,
    ):
        main()
...
def test_main_error_handling_order() -> None:
    ...
    with (
        patch("run.InstagramHelperGUI", side_effect=ImportError("Import failed")),
        patch("builtins.print") as mock_print,
        patch("sys.exit") as mock_exit,
    ):
        main()
    ...
    with (
        patch("run.InstagramHelperGUI", side_effect=Exception("General error")),
        patch("builtins.print") as mock_print,
        patch("sys.exit") as mock_exit,
    ):
        main()
