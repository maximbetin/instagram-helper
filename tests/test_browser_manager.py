...
def test_setup_browser_local_browser_success(mock_playwright: MagicMock, mock_browser: MagicMock) -> None:
...
def test_setup_browser_fallback_to_playwright(mock_playwright: MagicMock, mock_browser: MagicMock) -> None:
...
def test_setup_browser_all_attempts_fail(mock_playwright: MagicMock) -> None:
    ...
    with patch('browser_manager.settings') as mock_settings, \
         patch('browser_manager._launch_local_browser', return_value=None), \
         patch('browser_manager._launch_playwright_chromium', side_effect=Exception("Browser launch failed")):
...
def test_launch_local_browser_success(mock_playwright: MagicMock, mock_browser: MagicMock) -> None:
...
def test_launch_local_browser_other_connection_error(mock_playwright: MagicMock) -> None:
...
def test_launch_playwright_chromium_success(mock_playwright: MagicMock, mock_browser: MagicMock) -> None:
...
def test_kill_existing_browser_processes_brave_windows() -> None:
    """Test killing existing Brave processes on Windows."""
    with patch('browser_manager.settings') as mock_settings, \
         patch('subprocess.run') as mock_run:

        mock_settings.BROWSER_PATH = Path("C:/Program Files/BraveSoftware/Brave-Browser/Application/brave.exe")

        from browser_manager import _kill_existing_browser_processes
...
def test_kill_existing_browser_processes_taskkill_not_found() -> None:
    ...
    with patch('browser_manager.settings') as mock_settings, \
         patch('subprocess.run', side_effect=FileNotFoundError("taskkill.exe not found")):
...
def test_browser_launch_arguments(mock_playwright: MagicMock, mock_browser: MagicMock) -> None:
...
def test_playwright_chromium_launch_arguments(mock_playwright: MagicMock, mock_browser: MagicMock) -> None:
