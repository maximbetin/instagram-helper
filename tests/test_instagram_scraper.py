...
def test_get_post_urls_max_posts_limit(mock_page: MagicMock, scraper: InstagramScraper) -> None:
...
def test_get_post_date_from_url_post_type(mock_page: MagicMock, scraper: InstagramScraper) -> None:
    ...
    with patch.object(scraper, '_navigate_to_url', return_value=True), \
...
def test_get_post_date_from_url_reel_type(mock_page: MagicMock, scraper: InstagramScraper) -> None:
...
def test_get_post_date_from_url_unknown_type(mock_page: MagicMock, scraper: InstagramScraper) -> None:
...
def test_get_post_date_from_url_navigation_failure(mock_page: MagicMock, scraper: InstagramScraper) -> None:
...
def test_get_post_date_from_url_exception_handling(mock_page: MagicMock, scraper: InstagramScraper) -> None:
    """Test exception handling in post date extraction."""
    post_url = "https://www.instagram.com/test_account/p/123/"
    
    with patch.object(scraper, '_navigate_to_url', side_effect=Exception("Navigation error")):
        result = scraper._get_post_date_from_url(post_url)
        
        assert result is None


def test_extract_post_data_navigation_failure(mock_page: MagicMock, scraper: InstagramScraper) -> None:
    """Test post data extraction when navigation fails."""
    with patch.object(scraper, '_navigate_to_url', return_value=False):
        result = scraper._extract_post_data("https://example.com/p/123/", "test_account", datetime.now(UTC))
        
        assert result is None


def test_extract_post_data_no_date(mock_page: MagicMock, scraper: InstagramScraper) -> None:
...
def test_extract_post_data_old_post(mock_page: MagicMock, scraper: InstagramScraper) -> None:
    ...
    with patch.object(scraper, '_navigate_to_url', return_value=True), \
        ...
        result = scraper._extract_post_data("https://example.com/p/123/", "test_account", cutoff_date)
        
        assert result is None


def test_extract_post_data_success(mock_page: MagicMock, scraper: InstagramScraper) -> None:
...
def test_navigate_to_url_success(mock_page: MagicMock, scraper: InstagramScraper) -> None:
...
def test_navigate_to_url_timeout_retry(mock_page: MagicMock, scraper: InstagramScraper) -> None:
    """Test URL navigation with timeout retry."""
    # First attempt times out, second succeeds
    mock_page.goto.side_effect = [PlaywrightTimeoutError("Timeout"), None]
    
    result = scraper._navigate_to_url("https://example.com", "test operation", max_retries=1)
    
    assert result is True
    assert mock_page.goto.call_count == 2


def test_navigate_to_url_timeout_failure(mock_page: MagicMock, scraper: InstagramScraper) -> None:
    """Test URL navigation failure after all retries."""
...
def test_navigate_to_url_exception_failure(mock_page: MagicMock, scraper: InstagramScraper) -> None:
...
def test_get_post_date_no_element(mock_page: MagicMock, scraper: InstagramScraper) -> None:
...
def test_get_post_date_no_datetime_attribute(mock_page: MagicMock, scraper: InstagramScraper) -> None:
...
def test_get_post_date_parsing_error(mock_page: MagicMock, scraper: InstagramScraper) -> None:
...
def test_is_valid_post_url() -> None:
    ...
    assert InstagramScraper._is_valid_post_url("https://instagram.com/reel/123/") is True
    assert InstagramScraper._is_valid_post_url("https://instagram.com/story/123/") is False
...
def test_normalize_post_url(scraper: InstagramScraper) -> None:
    ...
    result = scraper._normalize_post_url("https://www.instagram.com/p/123/?utm_source=test")
    assert result == "https://www.instagram.com/p/123"
